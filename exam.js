let militaryUnit = {
  ...
  {
    return("name :" + militaryUnit.commandStructure.chiefOfStaff.name + " rank :"+ militaryUnit.commandStructure.chiefOfStaff.rank
        + " pone number: " + militaryUnit.commandStructure.chiefOfStaff.contact.phone
     ) 
  }

 function Mission2(militaryUnit)
 {
    //לא מוצא את מספר החיילים בקוד
    return("The number of soldiers in the unit: ")
 }



 function Mission3(militaryUnit, newMission) 
{
    militaryUnit.history.push({eventDate: militaryUnit.currentDeployment.estimatedEndDate,eventDescription: militaryUnit.currentDeployment.mission});       
    
    militaryUnit.currentDeployment = {newMission};
    return militaryUnit;
}

function Mission4(militaryUnit, newFirearm) {
    let firearmExists = false;

 
    for (let i = 0; i < militaryUnit.equipment.firearms.length; i++) {
        let existingFirearm = militaryUnit.equipment.firearms[i];


        if (existingFirearm.type === newFirearm.type && existingFirearm.status === newFirearm.status) {
...
const newFirearms ={type: "M4 Carbine",quantity: 200,status: "Operational"}
const newMission = { location: "Eastern Front" ,mission:"eliminate Hamas",startDate: "2024-07-01", endDate: "2024-12-31" };

 console.log(Mission1(militaryUnit))
 console.log(Mission3(militaryUnit))
